...
riscv_cfft_radix4_instance_q15  S_cfft_q15;
riscv_dct4_instance_q15 S_dct_q15;

q15_t state_q15[4*RFFT_LEN] = {0};  
riscv_status status;
int32_t main(void)
{
/*Init*/ 
/*rfft Init*/
  status = riscv_cfft_radix4_init_q15(&S_cfft_q15,RFFT_LEN/2,ifftFlag,doBitReverse);
  printf("status = %d\n",status);
  status = riscv_rfft_init_q15(&S_rfft_q15,RFFT_LEN,ifftFlag,doBitReverse); 	
  printf("status = %d\n",status);
  status = riscv_dct4_init_q15(&S_dct_q15, &S_rfft_q15, &S_cfft_q15, RFFT_LEN,RFFT_LEN/2, 4096);
  printf("status = %d\n",status);
/*Tests*/
/*dct*/
  perf_reset();
  perf_enable_id(EVENT_ID);
  riscv_dct4_q15(&S_dct_q15,state_q15, testInput_q15);
  perf_stop();
  printf("riscv_dct4_q15: %s: %d\n", SPR_PCER_NAME(EVENT_ID),  cpu_perf_get(EVENT_ID));
#ifdef PRINT_OUTPUT
  PRINT_Q(testInput_q15,RFFT_LEN);
#endif
  printf("End\n");

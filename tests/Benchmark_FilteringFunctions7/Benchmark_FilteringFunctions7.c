...
riscv_lms_instance_f32 S_lms_f32;
float32_t coeffs_lms_f32[NUMTAPS] =  {0.75, -0.4, 0.6, 0.8, -0.45,0.11};
float32_t state_lms_f32[NUMTAPS + MAX_BLOCKSIZE - 1u]; 
float32_t err_signal_f32[MAX_BLOCKSIZE];

riscv_lms_instance_q15 S_lms_q15;
q15_t coeffs_lms_q15[NUMTAPS] =  {0x7531, 0x3344,0xAA76, 0x01A1, 0x5C00,0x18}; 
q15_t state_lms_q15[NUMTAPS + MAX_BLOCKSIZE - 1u]; 
q15_t err_signal_q15[MAX_BLOCKSIZE];

riscv_lms_instance_q31 S_lms_q31;
q31_t coeffs_lms_q31[NUMTAPS] =  {0x7531, 0x3344,0xAA76, 0x01A1, 0x5C00,0x18}; 
q31_t state_lms_q31[NUMTAPS + MAX_BLOCKSIZE - 1u]; 
q31_t err_signal_q31[MAX_BLOCKSIZE];
/*Finite Impulse Response (FIR) Sparse Filters variables*/
int32_t pTapDelay[NUMTAPS] = {1 , 3 ,4,5, 7,8}; /*non zero indcies*/
riscv_fir_sparse_instance_f32 S_sparse_f32;
float32_t coeffs_sparse_f32[NUMTAPS] =  {0.75, -0.4, 0.6, 0.8, -0.45,0.11};
float32_t state_sparse_f32[MAX_BLOCKSIZE + MAXDELAY]; 
float32_t scratch_f32[MAX_BLOCKSIZE];

riscv_fir_sparse_instance_q7 S_sparse_q7;
q7_t coeffs_sparse_q7[NUMTAPS] =   {0x75, 0x33,0xAA, 0x01, 0x5C,0x18}; 
q7_t state_sparse_q7[MAX_BLOCKSIZE + MAXDELAY]; 
q7_t scratch_q7[MAX_BLOCKSIZE];

riscv_fir_sparse_instance_q15 S_sparse_q15;
q15_t coeffs_sparse_q15[NUMTAPS] =   {0x7531, 0x3344,0xAA76, 0x01A1, 0x5C00,0x18}; 
q15_t state_sparse_q15[MAX_BLOCKSIZE + MAXDELAY]; 
q15_t scratch_q15[MAX_BLOCKSIZE];

riscv_fir_sparse_instance_q31 S_sparse_q31;
q31_t coeffs_sparse_q31[NUMTAPS] =   {0x7531, 0x3344,0xAA76, 0x01A1, 0x5C00,0x18}; 
q31_t state_sparse_q31[MAX_BLOCKSIZE + MAXDELAY]; 
q31_t scratch_q31[MAX_BLOCKSIZE];
q31_t scratchout[MAX_BLOCKSIZE];
int i = 0 ;
...
int32_t main(void)
{
 ...
 /*Least Mean Square (LMS) Filters Init*/
  ...
  riscv_lms_init_q15(&S_lms_q15, NUMTAPS, coeffs_lms_q15, state_lms_q15, MU_q15, MAX_BLOCKSIZE,POS_SHIFT);
  riscv_lms_init_q31(&S_lms_q31, NUMTAPS, coeffs_lms_q31, state_lms_q31, MU_q31, MAX_BLOCKSIZE,POS_SHIFT);
 /*Finite Impulse Response (FIR) Sparse Filters Init*/
  riscv_fir_sparse_init_f32(&S_sparse_f32,NUMTAPS,coeffs_sparse_f32, state_sparse_f32, pTapDelay, MAXDELAY, MAX_BLOCKSIZE);
